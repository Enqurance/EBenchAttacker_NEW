...
def autodan_sample_control(control_suffixs, score_list, num_elites, batch_size, crossover=0.5,
                           num_points=5, mutation=0.01, openai_client=None, reference=None, if_softmax=True, if_api=True):
    ...
    offspring = apply_crossover_and_mutation(parents_list, crossover_probability=crossover,
                                                     ...
                                                     mutation_rate=mutation, openai_client=openai_client, reference=reference,
                                                     if_api=if_api)
...
def apply_crossover_and_mutation(selected_data, crossover_probability=0.5, num_points=3, mutation_rate=0.01,
    ...
    mutated_offspring = apply_gpt_mutation(offspring, mutation_rate, openai_client, reference, if_api)
...
def gpt_mutate(sentence, openai_client=None):
    ...
    while not received:
        try:
            response = openai_client.model.chat.completions.create(
            ...
            error_info = f"OpenAI API call failed due to {e}."
            print(error_info)
            time.sleep(1)
...
def apply_gpt_mutation(offspring, mutation_rate=0.01, openai_client=None, reference=None, if_api=True):
    if if_api:
...
def apply_init_gpt_mutation(offspring, mutation_rate=0.01, openai_client=None, if_api=True):
...
def autodan_sample_control_hga(word_dict, control_suffixs, score_list, num_elites, batch_size, crossover=0.5,
                               mutation=0.01, openai_client=None, reference=None, if_api=True):
    score_list = [-x for x in score_list]
    # Step 1: Sort the score_list and get corresponding control_suffixs
    sorted_indices = sorted(range(len(score_list)), key=lambda k: score_list[k], reverse=True)
    sorted_control_suffixs = [control_suffixs[i] for i in sorted_indices]
    ...
    offspring = apply_gpt_mutation(offspring, mutation, openai_client, reference, if_api)
...
def get_score_autodan(tokenizer, conv_template, instruction, target, model, device, test_controls=None, crit=None, dis=None):
    ...
    for item in test_controls:
        suffix_manager = autodan_SuffixManager(tokenizer=tokenizer,
                                               ...
                                               target=target,
                                               adv_string=item)
        input_ids, _ = suffix_manager.get_input_ids(adv_string=item)
...
def get_score_autodan_low_memory(tokenizer, conv_template, instruction, target, model, device, test_controls=None,
    ...
    for item in test_controls:
        suffix_manager = autodan_SuffixManager(tokenizer=tokenizer,
                                               ...
                                               adv_string=item)
        input_ids, _ = suffix_manager.get_input_ids(adv_string=item)
        input_ids_tensor = torch.stack([input_ids.to(device)], dim=0)
